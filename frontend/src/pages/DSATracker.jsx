...
const DSATracker = () => {
  ...
  const fetchHistory = async () => {
     try {
        const user = JSON.parse(localStorage.getItem('user'));
        if (!user) return;
        const res = await axios.get(`https://dsa-tracker-mxj5.onrender.com/api/dsa/${user.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('https://dsa-tracker-mxj5.onrender.com/api/dsa', {
        userId: user.id,
        problems
      });
  ...
  const handleEditSubmit = async () => {
      try {
          await axios.put(`https://dsa-tracker-mxj5.onrender.com/api/dsa/${editingProblem.logId}/problem/${editingProblem.problemId}`, editForm);
